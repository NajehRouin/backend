...
let componentCtrl = {
  AjouterComponet: async (req, res) => {
    try {
      ...
      if (!component)
        return res.status(302).json({ msg: "component  Obligatoire" });

      if (!matiere)
        return res.status(302).json({ msg: "matiere  Obligatoire" });
  ...
  regroupeComponet: async (req, res) => {
    try {
      let findComponent = await Component.find().populate("matiere");
      const groupedComponet = Object.values(
        findComponent.reduce((acc, item) => {
          const matiereNom = item.matiere.nom;
          if (!acc[matiereNom]) {
            acc[matiereNom] = { matiere: matiereNom, items: [] };
          }
          acc[matiereNom].items.push({
            index: item._id,
            nom: item.component,
            prix: item.prix,
          });
